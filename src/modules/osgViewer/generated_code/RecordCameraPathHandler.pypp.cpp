...
void register_RecordCameraPathHandler_class(){
    ...
    { //::osgViewer::RecordCameraPathHandler
        ...
        RecordCameraPathHandler_exposer_t RecordCameraPathHandler_exposer = RecordCameraPathHandler_exposer_t( "RecordCameraPathHandler", "\n Event handler allowing the user to record the animation path of a camera. In its current\n implementation, this handler cannot guarantee the final view matrix is correct; it is\n conceivable that the matrix may be one frame off. Eh--not a big deal! :)\n TODO: Write the file as we go, not when its all done.\n TODO: Create an osgviewer on-screen indication that animation is taking place.\n", bp::init< bp::optional< std::string const &, float > >(( bp::arg("filename")="saved_animation.path", bp::arg("fps")=2.5e+1f ), "\n Event handler allowing the user to record the animation path of a camera. In its current\n implementation, this handler cannot guarantee the final view matrix is correct; it is\n conceivable that the matrix may be one frame off. Eh--not a big deal! :)\n TODO: Write the file as we go, not when its all done.\n TODO: Create an osgviewer on-screen indication that animation is taking place.\n") );
